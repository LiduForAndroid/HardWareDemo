...
public abstract class BaseSerialPortDevice {
     ...
     * @return response for this command
     */
    public synchronized byte[] execute2(byte[] command, int timeToWait) throws IOException {
        byte[] bytes=readCMDs();
        if(bytes!=null&&bytes.length>0){
            //mDebugLogger.debug("card sender before data=="+HexUtil.GetHexString(bytes));
        }
        writeCMDs(command);
        long start=System.currentTimeMillis();
        int i=0;
        try {
            while(i*timeToWait<10000){
                Thread.sleep(timeToWait);
                i++;
                if(mSerialPort.getInputStream().available()>2) {
                    long end=System.currentTimeMillis();
                    long time = end-start;
                    if(i>2){
                       // mDebugLogger.debug("cmd=="+HexUtil.GetHexString(command)+";"+"time=="+time);
                    }
                    byte[] result = readCMDs();
                    Log.e("baseSerial","read cmds==="+HexUtil.GetHexString(result));
                    return result;
                }
            }
        } catch (InterruptedException e) {
